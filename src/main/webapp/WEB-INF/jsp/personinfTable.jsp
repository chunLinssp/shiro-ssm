    ...
    <script>
        $(function () {
            ...
            $("#addButton").click(function () {
                var name = $("#addPeoname").val();
                var sexTmp = $("input:radio[name='sex']:checked").val();
                var sex;
                if(sexTmp == "boy")
                    sex="男";
                else
                    sex="女";
                var id_card = $("#addIdcard").val();
                var address = $("#addAddress").val();
                var phone = $("#addPhone").val();
                var post_name = $("#addPostname").val();
                var departmentName = $("#adddep-name option:selected").val();
                var work_start =  $("#adddateInput").val();
                var send = {};
                send['name'] = name;
                send['sex'] = sex;
                send['id_card'] = id_card;
                send['address'] = address;
                send['phone'] = phone;
                send['post_name'] = post_name;
                send['departmentName'] = departmentName;
                send['work_start'] = work_start;

                $.ajax({
                    url:'/personinf/add.action',
                    type:"post",
                    contentType: 'application/json',
                    data:JSON.stringify(send),
                    success:function (data) {
        ...
        function changePage(currentPage) {
            $.get('/personinf/findAllByPage.action?pageNo='+currentPage,function (data) {
                var begin;
                var end;
...
'                                                    <c:if test="${it.key.indexOf(\'档案修改\')>-1}">\n' +
'                                                        <button type="button" class="btn btn-info"\n' +
'                                                                onclick="updatePersoninf(\''+obj.id+'\')">修改\n' +
'                                                        </button>\n' +
'                                                    </c:if>\n' +
'                                                    <c:if test="${it.key.indexOf(\'档案删除\')>-1}">\n' +
...
'                                        </tr>';
        ...
        function addPersoninf() {
            $("#addModal").modal('show');
        }
        function updatePersoninf(peo_id) {
            location.href = '/personinf/findById.action?id='+peo_id;
            // $.ajax({
            //     type: 'get',
            //     data: {
            //         id: peo_id
            //     },
            //     url: '/personinf/findById.action',
            //     success: function (data) {

                    // $("#myModal").modal('show');
                    // $("#Peonum").val(data.data.id);
                    // $("#Peoname").val(data.data.name);
                    // $("#Peosex").val(data.data.sex);
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <div>
                        <span>性别</span>
                        <input id="Peosex" type="text" class="form-control" placeholder="" aria-describedby="basic-addon1">
                    </div>
                    <div>
                        <span>职位</span>
                        <input id="Postname" type="text" class="form-control" placeholder="" aria-describedby="basic-addon1">
                    </div>
                    <div>
                        <span>部门</span>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div>
                        <span>名称</span>
                        <input id="addPeoname" type="text" class="form-control" placeholder="" aria-describedby="basic-addon1">
                    </div>
                    <div>
                        <span>性别</span>
                        <input type="radio" name="sex" id="optionsBoy"
                               value="boy" checked> 男
                        <input type="radio" name="sex" id="optionsGirl"
                               value="girl" checked> 女
                    </div>
                    <div>
                        <span>身份证</span>
                        <input id="addIdcard" type="text" class="form-control" placeholder="" aria-describedby="basic-addon1">
                    </div>
                    <div>
                        <span>家庭住址</span>
                        <input id="addAddress" type="text" class="form-control" placeholder="" aria-describedby="basic-addon1">
                    </div>
                    <div>
                        <span>联系电话</span>
                        <input id="addPhone" type="text" class="form-control" placeholder="" aria-describedby="basic-addon1">
                    </div>
                    <div>
                        <span>职位</span>
                        <input id="addPostname" type="text" class="form-control" placeholder="" aria-describedby="basic-addon1">
                    </div>
                    <div>
                        <span>部门</span>
                        <select class="form-control" id="adddep-name">
                            <c:forEach items="${departments}" var="item">
                                <option value="${item.name}">${item.name}</option>
                            </c:forEach>
                        </select>
        ...
        <div class="container-fluid">
            ...
            <div class="row page-titles">
                ...
                <div class="col-md-6 col-4 align-self-center">
                    <c:forEach items="${permissionVo.permissionNameAndUrl}" var="it">
                        <c:if test="${it.key.indexOf('人事档案添加')>-1}">
                            <a href="#" class="btn pull-right hidden-sm-down btn-success" onclick="addPersoninf()">${it.key}</a>
                        </c:if>
                    </c:forEach>
                </div>
            </div>
            ...
            <div class="row">
                                ...
                                <div class="col-sm-12">
                                    <div class="card">
                                        ...
                                        <h4 class="card-title">人事档案</h4>
                            <h5 class="card-subtitle" id="pageTotal">总条数:${pageInfo.total}</h5>
                                ...
                                <table class="table">
                                    ...
                                    <c:forEach items="${pageInfo.list}" var="item" varStatus="status">
                                        <tr>
                                            ...
                                            <td>
                                                <c:forEach items="${permissionVo.permissionNameAndUrl}" var="it">
                                                    <c:if test="${it.key.indexOf('修改')>-1}">
                                                        <button type="button" class="btn btn-info"
                                                                onclick="updatePersoninf('${item.id}')">修改
                                                        </button>
                                                    </c:if>
